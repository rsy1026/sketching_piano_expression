...
###############################################################
This code has been modified by "musicXML_parser(KAIST)" (~2018)
- in brief, DS(Dasaem Jeong, Ph.D in KAIST, mainly contributed)
###############################################################
...
class MusicXMLParserState(object):
  ...
  def __init__(self):
    ...
    # Keep track of current measure number. Does not support polymeter.
...
class Part(object):
  ...
  def _parse(self, xml_part, score_parts):
    ...
    for measure_number, measure in enumerate(xml_measures): # modified by DS
...
class Measure(object):
  ...
  def __init__(self, xml_measure, state):
    ...
    self.barline = None  # 'double' or 'final' or None --> added by DS
    self.repeat = None  # 'start' or 'stop' or None --> added by DS
  ...
  def __repr__(self): # added by SY
    measure_str = '<measure_number: {}>'.format(self.notes[0].measure_number)
    return measure_str
  ...
  def _parse(self):
    ...
    for child in self.xml_measure:
      ...
      elif child.tag == 'barline': # added by DS
      ...
      elif child.tag == 'grace': # grace note
        chord_symbol = ChordSymbol(child, self.state)
        self.chord_symbols.append(chord_symbol)
      else:
  ...
  def _parse_barline(self, xml_barline): # added by DS
...
class Direction(object): # added by DS
...
class Note(object):
  ...
  def __init__(self, xml_note, state):
    ...
    self.is_grace_note = False      # whether grace note
    self.pitch = None               # Tuple (Pitch Name, MIDI number)
    self.x_position = None # added by SY
    self.y_position = None # added by SY
    self.note_duration = NoteDuration(state)
    self.note_notations = NoteNotations() # added by DS
    self.staff = 1 # added by DS
    self.stem = 1 # added by SY
    self.measure_number = state.measure_number # added by DS
    self.state = state
    self._parse()

  def __repr__(self): # added by SY
    note_str = '<time: {} / pitch: {} / measure: {}>'.format(
      self.note_duration.time_position, self.pitch, self.measure_number)
    return note_str
  ...
  def _parse(self):
    ...
    if 'default-x' in self.xml_note.attrib.keys():
      self.x_position = float(self.xml_note.attrib['default-x'])
    elif 'default-x' in self.xml_note.attrib.keys():
      self.y_position = float(self.xml_note.attrib['default-y'])
    for child in self.xml_note:
      ...
      elif child.tag == 'notations': # added by DS
      ...
      elif child.tag == 'staff': # added by DS
        self.staff = int(child.text)
      elif child.tag == 'stem': # added by SY
        self.stem = child.text
      else:
        # Ignore other tag types because they are not relevant to Magenta.
        pass

    # reset state.chord_index if it is not chord note --> added by DS
  ...
  def pitch_to_midi_pitch(step, alter, octave):
    ...
    pitch_class = pitch_class + int(alter) # modified by DS 
    '''
    * modified to make B#4 == C5 / Cb5 == B4
      -1 % 12 = 11 -> octave should go down but becomes up due to "%"
      13 % 12 = 1 -> octave should go up but becomes down due to "%"
    '''
...
class NoteDuration(object):
  ...
  def __init__(self, state):
    ...
    self.xml_position = 0               # Onset time in duration
  ...
  def parse_duration(self, is_in_chord, is_grace_note, duration):
    ...
    if is_in_chord:
      ...
      self.xml_position = self.state.previous_note.note_duration.xml_position # added by DS
    else:
      # Only increment time positions once in chord
      self.state.time_position += self.seconds
      self.state.xml_position += self.duration # added by DS
...
class NoteNotations(object): # added by DS
  ...
  def __init__(self, xml_notations=None):
    ...
    self.is_trill = False
    self.tuplet_start = False # added by SY
    self.tuplet_stop = False # added by SY
    self.is_strong_accent = False
    self.is_staccatissimo = False
    self.wavy_line = None
  ...
  def parse_notations(self, xml_notations):
    ...
    if self.xml_notations is not None:
      ...
      for child in notations:
        ...
        elif child.tag == 'tuplet': # added by SY
          if child.attrib['type'] == 'start':
            self.tuplet_start = True
          elif child.attrib['type'] == 'stop':
            self.tuplet_stop = True
        elif child.tag == 'arpeggiate': # modified by SY
...
class TimeSignature(object):
  ...
  def __init__(self, state, xml_time=None):
    ...
    self.numerator = -1
    self.denominator = -1
    self.numerator_raw = -1 # added by SY
    self.denominator_raw = -1 # added by SY
    self.time_position = 0
    self.xml_position = 0
    self.state = state
  ...
  def _parse(self):
    ...
    try:
      ...
      self.numerator_raw = int(beats) # added by SY
      self.denominator_raw = int(beat_type) # added by SY
    except ValueError:
  ...
  def __eq__(self, other):
    isequal = self.numerator == self.numerator_raw == other.numerator
    isequal = isequal and (self.denominator == self.denominator_raw == other.denominator)
